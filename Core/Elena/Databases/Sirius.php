...
class Sirius{
    ...
    public function RawSQL(String $sql, Array $params)
    {
        self::$query = $sql;
        self::$stmt = self::$pdo->prepare(self::$query);
        foreach($params as $terms => $values){
            $types = [
            ...
            foreach($types as $key=>$pdo_mode) {
                if(gettype($terms) == $key){
                    self::$stmt->bindValue($terms, $values, $pdo_mode);
                    break;
                }
            }
        }
    ...
    public static function Table(String $table, Bool $debug = False)
    {
        ...
        if(trim($table) != "") {
            ...
            throw new ClearSkySiriusException('$table引数へ空白を渡すことは許可されていません。');
            exit(1);
        }
    }
    ...
    public function Column(Array $col)
    {
        ...
        foreach($col as $cols){
            ...
            if(trim($cols) == ""){
                throw new ClearSkySiriusException('$col引数の配列に空白を混ぜて渡すことは許可されていません。');
                exit(1);
            }
        }
        // カラム名の配列をプロパティに返却
    ...
    public function Where(String $Col, String $Is, String $Term, Bool $IsNot = False)
    {
        ...
        if(trim($Col) != "" and trim($Is) != "" and trim($Term) != "") {
            ...
            throw new ClearSkySiriusException('$Col引数、$Is引数、$Term引数それぞれに空白を渡すことは許可されていません。');
        }
    }
    ...
    public function And(String $Col, String $Is, String $Term, Bool $IsNot = False)
    {
        if(trim($Col) != "" and trim($Is) != "" and trim($Term) != "") {
            if(self::$WhereCalled) {
                ...
                throw new ClearSkySiriusException('Whereメソッド呼び出し前にAndメソッドを使用することは許可されていません。');
            }
        }else{
            throw new ClearSkySiriusException('$Col引数、$Is引数、$Term引数それぞれに空白を渡すことは許可されていません。');
        }
    ...
    {
        self::$GroupBy = ' GROUP BY '.implode(',', $table);
        return $this;
    }
    ...
    public function Or(String $Col, String $Is, String $Term, Bool $IsNot = False)
    {
        if(trim($Col) != "" and trim($Is) != "" and trim($Term) != "") {
            if(self::$WhereCalled) {
                ...
                throw new ClearSkySiriusException('Whereメソッド呼び出し前にOrメソッドを使用することは許可されていません。');
            }
        }else{
            throw new ClearSkySiriusException('$Col引数、$Is引数、$Term引数それぞれに空白を渡すことは許可されていません。');
        }
    ...
    public function Join(String $Table2Name, String $Table2ColName, String $TableColName, Bool $OUTER = False, Bool $LEFT = True)
    {
        if(trim($Table2Name) != "" and trim($Table2ColName) != "" and trim($TableColName) != ""){
            ...
            throw new ClearSkySiriusException('$Table2Name引数、$Table2ColName引数、$TableColName引数それぞれに空白を渡すことは許可されていません。');
        }
    }

    public function Fetch(Bool $IsOrderBy = False, String $OrderBy = "DESC", String $IsColumn = "Id")
    {
        $orderby = "";
        if($IsOrderBy){
            if(trim($OrderBy) != "" and trim($IsColumn) != ""){
                ...
                throw new ClearSkySiriusException('$IsOrderByフラグをTrueにした場合、$OrderBy引数、$IsColumn引数それぞれに空白を渡すことは許可されていません。');
            }
        }
        $cols = "";
        ...
        $cols = rtrim($cols, ",");
        self::$query = "SELECT ".rtrim(trim($cols), ",")." FROM ".trim(self::$table).self::$JoinQuery.self::$TermsQuery.$orderby.self::$GroupBy;
        self::$stmt = self::$pdo->prepare(self::$query);
    ...
    public function All()
    {
        return (object)self::$stmt->fetchAll();
    }

    public function One()
    {
        return (object)self::$stmt->fetch();
    }
    ...
    public function GetParams(Bool $view=False){
        if($view){
            Debug::VarDumpBeautify(self::$terms);
        }else{
            return self::$terms;
        }
    }
