...
class Session{
    ...
    public static function Start(){
        if(self::Breath()){
            ...
            if($session_dumper == "RDB"){
                ...
                Sirius::Table("CSess")->Insert($system_array, False);
    ...
    public static function Delete(){
        ...
        $session_dumper = $settings["SessionDumper"];
        if($session_dumper == "RDB"){
            $Session = Sirius::Table("Csess")->Fetch(True)[0];
        }else if($session_dumper == "Redis"){
    ...
    public static function Breath(){
        ...
        $session_dumper = $settings["SessionDumper"];
        if($session_dumper == "RDB"){
            if(isset($_COOKIE["cndrl_sID"]) && Sirius::Table("Csess")->Where("cndrl_sID", "=", $_COOKIE["cndrl_sID"])->CountRow() != 0){
                return true;
            }else{
                return false;
            }
        ...
        if(self::Breath()){
            if($session_dumper == "RDB"){
                ...
                if(is_array($data)){
                    ...
                    Sirius::Table("Csess")->Where("cndrl_sID", "=", $session_id)->Update([$session_id => json_encode($data)]);
                }
            }else if($session_dumper == "Redis"){
                $session_id = "";
                if(isset($_COOKIE["cndrl_sID"])){
    ...
    public static function Reader(String $String){
        ...
        if(self::Breath()){
            if($session_dumper == "RDB"){
                ...
                $data = Sirius::Table("Csess")->Where("cndrl_sID", "=", $session_id)->Fetch()[0];
                $data = mb_convert_encoding($data, 'UTF8', 'ASCII,JIS,UTF-8,EUC-JP,SJIS-WIN');
    ...
    public static function AllBack(String $Key){
        ...
        if(self::Breath()){
            if($session_dumper == "RDB"){
                $data = Sirius::Table("Csess")->Where("cndrl_sID", "=", $Key)->Fetch()[0];
                $data = mb_convert_encoding($data, 'UTF8', 'ASCII,JIS,UTF-8,EUC-JP,SJIS-WIN');
